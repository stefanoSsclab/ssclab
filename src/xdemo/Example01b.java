...
public class Example01b {
	...
	public static void main(String[] args) throws Exception {
		...
		InputFile file = new InputFile("c:/ssc/demo_info/dati_testo/gente.txt"); 
		
		
		//specifico il formato di input dei dati presenti nel file 
		file.setInputFormat("NOME:varstring(20), COGNOME:varstring(20), NASCITA:date(gg/mm/aaaa), CELIBE:boolean, ALTEZZA:float, SESSO:singlechar[37]");

				
		//alloco una libreria con nome logico "LIB_DATI" che corrisponde al path fisico "f:\\demo\\libreria_dati"
		session.addLibrary("LIB_DATI",connOracle());
		...
		DataStep datastep = session.createDataStep("LIB_DATI.PERSONE8", file); 
		datastep.setDropVarOutput("CELIBE");
		...
		FileStep filestep = session.createFileStep("c:\\ssc\\demo_info\\output\\stampa_gente.txt", "LIB_DATI.PERSONE8");
		filestep.printf("%12s %-15s %tD %03.2f %S  ", "COGNOME","NOME", "NASCITA", "ALTEZZA", "SESSO");
		filestep.execute();
